...
SearchRet binarySearchByID(int, Student[], int);
SearchRet binarySearchByName(string, Student[], int);
Student* bubbleSortArray(Student[], int);
void displayStudentArray(Student);
void displayStudentArray(Student[], int);
Student FindFast(int, Student[], int);
Student FindFast(string, Student[], int);
...
int main()
{
    Student  students[] =
        {
            ...
            {5,"Brooke Cazares","SÃ£o JosÃ© dos Campos",'F',21,3.7},
            {6,"Rochelle Johnson","Indianapolis",'F',21,3.4},
            {7,"Joey Abreu","Shenyang",'M',22,3.7},
            {8,"Preston Suarez","SÃ£o Paulo",'M',22,3.8},
            ...
            {51,"Issac Mata","TehuacÃ¡n",'M',23,2.4},
            ...
            {97,"Mateo Cisneros","Foz do IguaÃ§u",'M',23,3},
            ...
            {129,"Brandy Waldner","ZÃ¼lpich",'F',20,3.2},
            ...
            {136,"Ivy Lucero","SÃ£o Paulo",'F',24,3.9},
            ...
            {142,"Stephen Wharton","Tuxtla GutiÃ©rrez",'M',21,2.1},
            ...
            {184,"Isabella Martinez","BogotÃ¡",'F',23,2.6},
            {185,"Kimberly Thielke","Los Angeles",'F',21,3.8},
            {186,"Adrian Miguel-Gomez","MedellÃ­n",'M',22,3.7},
            ...
            {227,"Abigail Gallegos","BelÃ©m",'F',24,3.4},
            ...
            {272,"Jasmine Lopez","GuaxupÃ©",'F',21,3.8},
            ...
            {284,"Jesse Carballo","BogotÃ¡",'M',20,3.8},
            ...
            {302,"Vanessa Gonzalez","LeÃ³n",'F',24,3.3},
    ...
    Student  studentsCopy[STUDENT_SIZE];
        ...
        while (true)
        {
            ...
            cout << "1. Search by ID Fast" << endl;
            cout << "2. Search by Name Fast" << endl;
            cout << "0. Exit" << endl;
            cout << "Enter [0-2]: ";
            ...
            switch (response) {
                case 1:
                    ...
                    searchedStudent = FindFast(studentID, students, STUDENT_SIZE);
                    if (searchedStudent.Id != 0) {
                        displayStudentArray(students, searchedStudent.Id);
                    } else {
                ...
                case 2:
                    ...
                    cin >> studentFName >> studentLName;
                    studentFullName = studentFName + " " + studentLName;
                    
                    bubbleSortArray(students, STUDENT_SIZE);
                    searchedStudent = FindFast(studentFullName, students, STUDENT_SIZE);
                    
                    if (searchedStudent.Id != 0) {
                        displayStudentArray(searchedStudent);
                    } else {
                        cout << "Student with Student Name " << searchedStudent.Name << " Not found." << endl;
                    }
                    break;
                default:
                    cout << "Wrong input!";
...
void displayStudentArray(Student student) {
    cout << "{" << student.Id << ",";
    cout << "\"" << student.Name;
    cout << "\",\"" << student.City;
    cout << "\",'" << student.Gender;
...
void displayStudentArray(Student students[], int student_index_found)
{
    cout << "{" << students[student_index_found].Id << ",";
    cout << "\"" << students[student_index_found].Name;
    cout << "\",\"" << students[student_index_found].City;
    cout << "\",'" << students[student_index_found].Gender;
    cout << "'," << students[student_index_found].Age;
    cout << "," << students[student_index_found].GPA << "}" << endl;
}

SearchRet binarySearchByID(int studentID, Student students[], int student_size)
{
    int first = 0;                // First element of list
    int last = student_size - 1;  // last element of the list
    int middle;                   // variable containing the current middle value of the list
    int numComparisons = 0;         // counter for number of comparisons
    SearchRet ret = { -1, numComparisons };
    
    while (first <= last)
    {
        middle = first + (last - first) / 2;

        if (students[middle].Id == studentID - 1) {
            ret = { middle, numComparisons };
            return ret;        // if value is in the middle, we are done
            
        } else if (students[middle].Id < studentID - 1) {
            first = middle - 1;    // toss out the first remaining half of
            numComparisons++;
            
        } else {
            last = middle + 1;    // toss out the second remaining half of the array and search the second
            numComparisons++;
        }
    }
...
SearchRet binarySearchByName(string studentName, Student students[], int student_size)
{
    int first = 0;                // First element of list
    int last = student_size - 1;  // last element of the list
    int middle;                   // variable containing the current middle value of the list
    ...
    while (first <= last)
    {
        ...
        if (students[middle].Name == studentName) {
            ret = { middle, numComparisons };
            return ret;        // if value is in the middle, we are done
            
        } else if (students[middle].Name < studentName) {
            first = middle -1;
            numComparisons++;
...
Student* bubbleSortArray(Student students[], int student_size)
{
    bool swap;
    Student temp;
    int bottom;
    bottom = student_size - 1;    // bottom indicates the end part of the array where the smallest values have settled in order
    do
    {
        swap = false;
        ...
        {
            if (students[index].Name > students[index + 1].Name)
            {
...
Student FindFast(int id, Student students[], int student_size)
{
    ...
    if (binarySearchByID(id, students, student_size).index != -1) {
        ret = students[binarySearchByID(id, students, student_size).index];
    }
    cout << "The number of comparisons were performed: " << binarySearchByID(id, students, student_size).numComparison << endl;
    return ret;
}

Student FindFast(string name, Student students[], int student_size)
{
    Student ret = {0, "Not found"};

    // Fill in the logic to find student by name
    // using binary search.
    // You will have to sort the data by name before
    // doing the find. Write a function that will sort
    // Student array based on name and call the function
    // before calling FindFast by name.
    if (binarySearchByName(name, students, student_size).index != -1) {
        ret = students[binarySearchByName(name, students, student_size).index];
    }
    cout << "The number of comparisons were performed: " << binarySearchByName(name, students, student_size).numComparison << endl;
    return ret;
}
